...
//Recibida de datos
$id = (isset($_GET['idRecetaExamen'])) ? $_GET['idRecetaExamen'] : 0;

//Instancia al controlador
require_once "../controladores/recetaExamenControlador.php";
$controlador = new recetaExamenControlador();
$datos = $controlador->reporte_receta_examen_controlador($id);
...
if ($datos->rowCount() >= 1) {
	...
	$pdf->SetTextColor(0, 107, 181);
	$pdf->Cell(0, 10, utf8_decode(strtoupper("Receta de examen")), 0, 0, 'C');
	...
	$pdf->Cell(0, 8, utf8_decode('Datos de la receta de examen'), 0, 0, 'C');
	$pdf->Ln(8);

	$pdf->SetFont('Arial', 'B', 12);
	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell($leftTitleMargin, 8, utf8_decode('Codigo receta examen: '), 0, 0);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell($descriptionOfLeftTitle, 8, utf8_decode($id), 0, 0);
	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell($rightTitleMargin, 8, utf8_decode('Consulta:'), 0, 0);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell($descriptionOfRightTitle, 8, utf8_decode($datos['ConsultaPrevia']), 0, 0);
	$pdf->Ln(8);
	...
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell($descriptionOfRightTitle, 8, utf8_decode($datos['nombreTipoExamen']), 0, 0);
	$pdf->Ln(8);
	...
	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell(25, 8, utf8_decode('Motivo:'), 0, 0);
	$pdf->Ln(8);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell(200, 8, utf8_decode($datos['Motivo']), 0, 0);
	$pdf->Ln(8 + 2);
