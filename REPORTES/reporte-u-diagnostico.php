...
//Recibida de datos
$id = (isset($_GET['idDiagnostico'])) ? $_GET['idDiagnostico'] : 0;

//Instancia al controlador
require_once "../controladores/diagnosticoControlador.php";
$controlador = new diagnosticoControlador();
$datos = $controlador->reporte_diagnostico_controlador($id);
...
if ($datos->rowCount() >= 1) {
	...
	$pdf->SetTextColor(0, 107, 181);
	$pdf->Cell(0, 10, utf8_decode(strtoupper("Reporte de diagnostico")), 0, 0, 'C');
	...
	$pdf->Cell(-35, 10, utf8_decode("R_D-00001"), '', 0, 'C');
	$pdf->Ln(10);
	...
	$pdf->Cell(0, 8, utf8_decode('Datos del diagnostico'), 0, 0, 'C');
	$pdf->Ln(8);

	$pdf->SetFont('Arial', 'B', 12);
	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell($leftTitleMargin, 8, utf8_decode('Codigo diagnostico'), 0, 0);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell($descriptionOfLeftTitle, 8, utf8_decode($id), 0, 0);
	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell($rightTitleMargin, 8, utf8_decode('Consulta:'), 0, 0);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell($descriptionOfRightTitle, 8, utf8_decode($datos['CodConsulta']), 0, 0);
	$pdf->Ln(8);

	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell($leftTitleMargin, 8, utf8_decode('Enfermedad:'), 0, 0);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell($descriptionOfLeftTitle, 8, utf8_decode($datos['NombreEnfermedad']), 0, 0);
	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell($rightTitleMargin, 8, utf8_decode('Tipo:'), 0, 0);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell($descriptionOfRightTitle, 8, utf8_decode($datos['TipoEnfermedad']), 0, 0);
	$pdf->Ln(8);
	...
	$pdf->Ln(8);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell(200, 8, utf8_decode($datos['Descripcion']), 0, 0);
	$pdf->Ln(8);
	
	$pdf->SetTextColor(33, 33, 33);
	$pdf->Cell(25, 8, utf8_decode('Nota diagnostico:'), 0, 0);
	$pdf->Ln(8);
	$pdf->SetTextColor(97, 97, 97);
	$pdf->Cell(200, 8, utf8_decode($datos['Nota']), 0, 0);
	$pdf->Ln(8);
