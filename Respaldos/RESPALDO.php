<?php
	...
	$db_name = 'bdcleanfull'; //Nombre de la Base de datos
...
if (php_sapi_name() != "cli") {
    ...
    $(document).ready(function(){
        window.open(GOOGLE.COM, "_blank"); // will open new tab on document ready
    });
  </script></div>';
}

//LOCURITAS PARA DESCARGAR
/*
$db_name = 'bdcleanfull'; //Nombre de la Base de datos
$fecha = date("Ymd-His");
$zip = new ZipArchive(); //Objeto de Libreria ZipArchive
	...
	if($zip->open($salida_zip,ZIPARCHIVE::CREATE)===true) { //Creamos y abrimos el archivo ZIP
		$zip->addFile($output); //Agregamos el archivo SQL a ZIP
		$zip->close(); //Cerramos el ZIP
		header ("Location: $salida_zip"); // Redireccionamos para descargar el Arcivo ZIP
		} else {
		echo 'Error'; //Enviamos el mensaje de error
